...
class Tile {
    constructor(instance, backgroundColor, foregroundColor, baseColor) {
        this.instance = instance;
        this.backgroundColor = backgroundColor;
        this.foregroundColor = foregroundColor;
        this.baseColor = baseColor;
    }
    toHexValue(variable, consumer) {
    ...
    applyColors(fullName) {
        const fills = this.instance.fills;
        if (fills && fills.length > 0 && fills[0].type === 'SOLID') {
            const updatedFill = figma.variables.setBoundVariableForPaint(fills[0], 'color', this.backgroundColor.variable);
            this.instance.fills = [updatedFill];
        }
        const tileText = this.instance.findOne(node => node.name === "text");
        if (tileText) {
        ...
        if (textFills && textFills.length > 0 && textFills[0].type === 'SOLID') {
            if (this.instance.name === 'gridHeaderTile') {
                const backgroundRGB = this.backgroundColor.variable.resolveForConsumer(this.instance).value;
                const bgPaint = { color: { r: backgroundRGB.r, g: backgroundRGB.g, b: backgroundRGB.b }, opacity: backgroundRGB.a };
                ...
                const hexText = this.instance.findOne(node => node.name === "hex");
                if (hexText) {
                    this.toHexValue(this.foregroundColor.variable, this.instance).then(hex => {
                        hexText.characters = backgroundRGB.a < 1 ? `${hex} ${Math.round(backgroundRGB.a * 100)}%`.toUpperCase() : hex.toUpperCase();
                        this.applyTextColor(hexText, blendedBgColor);
                    });
                }
            }
            else {
                const updatedTextFill = figma.variables.setBoundVariableForPaint(textFills[0], 'color', this.foregroundColor.variable);
                tileText.fills = [updatedTextFill];
            }
        }
    }
    applyTextColor(textNode, backgroundRGB) {
        const blackRGB = { r: 0, g: 0, b: 0 };
        const whiteRGB = { r: 1, g: 1, b: 1 };
        const blackContrast = this.getContrast({ color: blackRGB, opacity: 1 }, { color: backgroundRGB, opacity: 1 }, this.baseColor);
        const whiteContrast = this.getContrast({ color: whiteRGB, opacity: 1 }, { color: backgroundRGB, opacity: 1 }, this.baseColor);
    ...
    applyContrastLabel() {
        const bgPaint = this.instance.fills[0];
        const tileText = this.instance.findOne(node => node.name === "text");
        const fgPaint = tileText.fills[0];
        const contrast = this.getContrast(fgPaint, bgPaint, this.baseColor);
        const ratioLabel = this.instance.findOne(node => node.name === "ratio");
    ...
    getBlendedColor(paint, baseOpacity, baseColor) {
    ...
    applyVariant(contrast) {
        const label = this.instance.findOne(node => node.name === "label");
        if (label) {
    ...
    checkAndHideTile() {
        this.instance.fills = [];
        const nestedLayers = this.instance.findAll(node => node !== this.instance);
    ...
    getContrast(foreground, background, baseColor) {
        ...
        const blendedFgLuminance = this.getRelativeLuminance(blendedFgColor.r, blendedFgColor.g, blendedFgColor.b);
        const blendedBgLuminance = this.getRelativeLuminance(blendedBgColor.r, blendedBgColor.g, blendedBgColor.b);
        const [bright, dark] = blendedFgLuminance > blendedBgLuminance ? [blendedFgLuminance, blendedBgLuminance] : [blendedBgLuminance, blendedFgLuminance];
        return (bright + 0.05) / (dark + 0.05);
    }
    getContrastVariant(contrast) {
        if (contrast >= 7)
            return 'aaa';
        if (contrast >= 4.5)
            return 'aa';
...
class ColorMatrixGenerator {
    ...
    createFrame(layoutMode, name, itemSpacing, horPad, verPad, baseColor) {
    ...
    generateMatrix(backgroundGroups, foregroundGroups, array, baseColorHex) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!backgroundGroups.length || !foregroundGroups.length) {
                figma.notify('Select at least one variable group to generate the matrix.', { error: true });
                figma.ui.postMessage({ type: 'generation-error', reason: 'empty-selection' });
                return;
            }
            const allVariables = yield this.getColorVariables();
            ...
            foregroundGroups.forEach((group) => {
                if (groupedVariables[group]) {
                    this.foregroundColors.push(...groupedVariables[group]);
                }
            });
            const backgroundLabel = backgroundGroups.length ? backgroundGroups.join(', ') : 'background';
            const foregroundLabel = foregroundGroups.length ? foregroundGroups.join(', ') : 'text';
            const matrixLabel = array.useDistinct
                ? `${backgroundLabel} × ${foregroundLabel}`
                : backgroundLabel;
            const gridHeaderTileComponent = yield figma.importComponentByKeyAsync('d4136c7cdbfa21a9bc90421b055b2bd6f2260a54');
            const gridAxisTileComponent = yield figma.importComponentByKeyAsync('13d3d4c5a355847f9f56eaf96541e6fa8a29311d');
            const specHeaderTemplate = yield figma.importComponentByKeyAsync('e0b50d04f34885e7b7dc13680776a1a62c307006');
            const gridTileComponent = yield figma.importComponentByKeyAsync('f9ab15001530fa4cc6360b7f1742dc71f16a7add');
            ...
            const matrixFrame = this.createFrame('VERTICAL', matrixLabel.toLowerCase(), 0, 0, 0, baseColor);
            const colorFrame = this.createFrame('VERTICAL', 'colors', 1, 1, 1, baseColor);
            if (specHeaderTemplate) {
                ...
                const headerLabel = specHeaderCopy.findOne(node => node.name === 'title');
            ...
            if (gridTileComponent && gridTileComponent.type === 'COMPONENT') {
                const componentNode = gridTileComponent;
                const componentHeaderNode = gridHeaderTileComponent;
                const componentAxisNode = gridAxisTileComponent;
                const rowFrames = [];
                if (gridHeaderTileComponent && gridHeaderTileComponent.type === 'COMPONENT') {
                    const axisTileInstance = componentAxisNode.createInstance();
                    const headerFrame = this.createFrame('HORIZONTAL', 'header', 1, 0, 0, baseColor);
                    rowFrames.push(headerFrame);
                    headerFrame.appendChild(axisTileInstance);
                    for (let col = 0; col < this.foregroundColors.length; col++) {
                        const headerTileInstance = componentHeaderNode.createInstance();
                        const columnColor = this.foregroundColors[col];
                        const headerTile = new Tile(headerTileInstance, columnColor, columnColor, baseColor);
                ...
                for (let row = 0; row < this.backgroundColors.length; row++) {
                    const rowFrame = this.createFrame('HORIZONTAL', 'row', 1, 0, 0, baseColor);
                    rowFrames.push(rowFrame);
                    const headerRowTileInstance = componentHeaderNode.createInstance();
                    const rowColor = this.backgroundColors[row];
                    const headerRowTile = new Tile(headerRowTileInstance, rowColor, rowColor, baseColor);
                    headerRowTile.applyColors(array.fullName);
                    rowFrame.appendChild(headerRowTileInstance);
                    for (let col = 0; col < this.foregroundColors.length; col++) {
                        const tileInstance = componentNode.createInstance();
                        const tile = new Tile(tileInstance, this.backgroundColors[row], this.foregroundColors[col], baseColor);
                ...
                figma.closePlugin("Template components not found or not a valid component!");
            }
        });
    }
}
...
figma.ui.onmessage = (msg) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (msg.type === 'generate-grid') {
        const backgrounds = (_a = msg.array.selectedBackgroundGroups) !== null && _a !== void 0 ? _a : [];
        const foregrounds = msg.array.useDistinct
            ? (_b = msg.array.selectedForegroundGroups) !== null && _b !== void 0 ? _b : [] : backgrounds;
        yield generator.generateMatrix(backgrounds, foregrounds, msg.array, msg.baseColorHex);
    }
