...
class Tile {
  ...
  constructor(instance: InstanceNode, backgroundColor: ColorVariable, foregroundColor: ColorVariable, baseColor: RGB) {
  ...
  applyColors(fullName: boolean) {
    ...
    const tileText = this.instance.findOne(node => node.name === "text") as TextNode;
    if (tileText) {
    ...
    if (textFills && textFills.length > 0 && textFills[0].type === 'SOLID') {
      ...
      if (this.instance.name === 'gridHeaderTile') {
        const backgroundRGB = this.backgroundColor.variable.resolveForConsumer(this.instance).value as RGBA;
        const bgPaint = { color: { r: backgroundRGB.r, g: backgroundRGB.g, b: backgroundRGB.b }, opacity: backgroundRGB.a } as SolidPaint;
        ...
        const hexText = this.instance.findOne(node => node.name === "hex") as TextNode;
        if (hexText) {
  ...
  applyTextColor(textNode: TextNode, backgroundRGB: RGB) {
    ...
    const blackContrast = this.getContrast({ color: blackRGB, opacity: 1 } as SolidPaint, { color: backgroundRGB, opacity: 1 } as SolidPaint, this.baseColor);
    const whiteContrast = this.getContrast({ color: whiteRGB, opacity: 1 } as SolidPaint, { color: backgroundRGB, opacity: 1 } as SolidPaint, this.baseColor);
  ...
  applyContrastLabel() {
    ...
    const ratioLabel = this.instance.findOne(node => node.name === "ratio") as TextNode;
  ...
  applyVariant(contrast: number) {
    const label = this.instance.findOne(node => node.name === "label") as InstanceNode;
    if (label) {
  ...
  getRelativeLuminance(r: number, g: number, b: number): number {
    const linear = (x: number) => (x <= 0.03928 ? x / 12.92 : Math.pow((x + 0.055) / 1.055, 2.4));
    return 0.2126 * linear(r) + 0.7152 * linear(g) + 0.0722 * linear(b);
  }

  getContrast(foreground: SolidPaint, background: SolidPaint, baseColor: RGB): number {
  ...
  processTile(array: { aaa: boolean; aa: boolean; aa18: boolean; dnp: boolean }) {
...
class ColorMatrixGenerator {
  ...
  groupVariablesByPrefix(variables: ColorVariable[]): Record<string, ColorVariable[]> {
  ...
  createFrame(layoutMode: 'HORIZONTAL' | 'VERTICAL', name: string, itemSpacing: number, horPad: number, verPad: number, baseColor: RGB): FrameNode {
  ...
  async generateMatrix(
    ...
    const gridHeaderTileComponent = await figma.importComponentByKeyAsync('d4136c7cdbfa21a9bc90421b055b2bd6f2260a54');
    const gridAxisTileComponent = await figma.importComponentByKeyAsync('13d3d4c5a355847f9f56eaf96541e6fa8a29311d');
    const specHeaderTemplate = await figma.importComponentByKeyAsync('e0b50d04f34885e7b7dc13680776a1a62c307006');
    const gridTileComponent = await figma.importComponentByKeyAsync('f9ab15001530fa4cc6360b7f1742dc71f16a7add');
    ...
    if (gridTileComponent && gridTileComponent.type === 'COMPONENT') {
      ...
      if (gridHeaderTileComponent && gridHeaderTileComponent.type === 'COMPONENT') {
        const axisTileInstance = componentAxisNode.createInstance() as InstanceNode;
        ...
        for (let col = 0; col < this.foregroundColors.length; col++) {
          const headerTileInstance = componentHeaderNode.createInstance() as InstanceNode;
      ...
      for (let row = 0; row < this.backgroundColors.length; row++) {
        ...
        const headerRowTileInstance = componentHeaderNode.createInstance() as InstanceNode;
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'generate-grid') {
    ...
    await generator.generateMatrix(backgrounds, foregrounds, msg.array, msg.baseColorHex);
